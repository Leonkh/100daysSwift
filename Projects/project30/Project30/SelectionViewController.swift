...
class SelectionViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        for picture in items { // challenge 3
        let imageRootName = picture.replacingOccurrences(of: "Large", with: "Thumb")
...
//            ctx.cgContext.setShadow(offset: .zero, blur: .zero, color: nil)
            ...
            saveImage(image: rounded, name: picture)
        }
    }
...
//		}
        ...
        cell.imageView?.image = loadImage(image: currentImage)
...
//		viewControllers.append(vc)
    ...
    func loadImage(image: String) -> UIImage? {
        ...
        if let pick = UIImage(contentsOfFile: path.path) {
            return pick } else {
                return nil
            }
    }
