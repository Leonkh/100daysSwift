...
class GameScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        addChild(scoreLabel) // Challenge 1
    ...
    func explode(firework: SKNode) {
        if let emmiter = SKEmitterNode(fileNamed: "explode") {
            ...
            let removeFirework = SKAction.run {
                firework.removeFromParent()
            }
            let removeExplode = SKAction.run {
                emmiter.removeFromParent()
            }
            let explodeDelay = SKAction.sequence([removeFirework,delay, removeExplode]) // Challenge 3
            run(explodeDelay)
        }
